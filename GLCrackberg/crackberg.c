...
static BOOL crack = TRUE, boring = FALSE, nowater = FALSE, flat = TRUE, lit = TRUE, letterbox = FALSE;
static float visibility = 0.6;
static char *color = DEF_COLOR;
...
static Trile *trile_alloc(cberg_state *cberg)
{
    ...
    if (cberg->free_head) {
        ...
        ++cberg->count;
	   if ((new = IExec->AllocVec(sizeof(Trile), MEMF_CLEAR))
		 &&	(new->l = (double *) IExec->AllocVec(sizeof(double) * cberg->epoints * 3, MEMF_CLEAR))) {
...
{
	tr->morph_data = (void *) IExec->AllocVec(sizeof(double), MEMF_CLEAR);
	*((double *)tr->morph_data) = 0.02; /* not 0; avoid normals crapping */
}

static void grow_free(Trile *tr)
{
	IExec->FreeVec(tr->morph_data);
}
...
void reshape_crackberg (int w, int h);
...
void init_crackberg ()
{
    ...
    if (!cbergs) {
        ...
        if ( !(cbergs = IExec->AllocVec(sizeof(cberg_state), MEMF_CLEAR)))
        {
            exit(1);
        }
	...
	cberg->heights = IExec->AllocVec(cberg->tpoints * sizeof(double), MEMF_CLEAR);
	cberg->norms = IExec->AllocVec(3 * cberg->tnorms * sizeof(double), MEMF_CLEAR);
...
void draw_crackberg ()
{
    ...
    ITimer->GetSysTime(&cur_frame_t);
    cur_frame = (float)cur_frame_t.Seconds + (float)cur_frame_t.Microseconds * 1.0E-3;
...
/* uh */
void release_crackberg ()
{
	if (cbergs) {
		cberg_state *cberg = &cbergs[0];
		if (cberg->norms) {
			IExec->FreeVec((&cbergs[0])->norms);
		}
		IExec->FreeVec((&cbergs[0])->heights);
		cbergs = 0;
	}
}
