...
static void complain (int wanted_colors, int got_colors)
{
  if (got_colors > wanted_colors - 10)
	...
	IExec->DebugPrintF("Wanted %d colors; got %d.\n", wanted_colors, got_colors);
}



void make_color_ramp (Display *dpy, Colormap cmap,
		 ...
		 Bool writable_p)
{
 ...
 AGAIN:
  ...
  IUtility->SetMem(colors, 0, (*ncolorsP) * sizeof(*colors));
...
/*	XStoreColors (dpy, cmap, colors2, ncolors);
  XFlush(dpy);*/
  IExec->CopyMem(colors2, colors, sizeof(*colors) * ncolors);
  IExec->FreeVec(colors2);
}
