...
static GLfloat speed = 0.4f;  /* jwz -- overall speed factor applied to all motion */
static int cfg_balls = 25;
static GLfloat cfg_ballsize = 2.0f;
static GLfloat cfg_explosion = 15.0f;
static GLfloat cfg_decay = 0.1f;
...
{
   if (!t) return;
   if (t->tris) IExec->FreeVec(t->tris);
   if (t->vertices) IExec->FreeVec(t->vertices);
   if (t->normals) IExec->FreeVec(t->normals);
   t->tris = NULL;
   t->vertices = NULL;
   t->normals = NULL;
   t->num_tri = 0;
 ...
 */
void reshape_boxed(int width, int height)
{
...
void init_boxed(struct BlankerData* bd)
{
	/* Colormap    cmap; */
	...
	speed = bd->speed;
	cfg_balls = bd->balls;
	cfg_ballsize = bd->ballSize;
	cfg_explosion = bd->explosion;
	cfg_decay = bd->decay;
...
void release_boxed()
{
	...
	IExec->FreeVec (gp->bman.balls);
	IExec->FreeVec (gp->tman);
	IExec->FreeVec (gp->tex1);
}
/*********************************************************/
